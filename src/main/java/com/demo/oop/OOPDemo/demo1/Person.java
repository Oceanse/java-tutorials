 ...
 * 面向对象的语言不仅使用类来封装一类事物的内部状态数据(属性)，还会为这类事物的行为特征提供相应的实现(方法)
 * 成员变量（状态数据）＋方法（行为）＝类定义
 * <p>
 * <p>
...
public class Person {
    ...
    /**
     * 把A类的对象作为B类的成员变量组合起来，从而可以复用A类的属性和方法(组合或者聚合关系，has-a关系)
     */
    private Site site;
     ...
     * @param site
     */
    public Person(String name, String sex, String[] hobby, Site site) {
        this.name = name;
        this.sex = sex;
        this.hobby = hobby;
        this.site = site;
...
class Tests {
    public static void main(String[] args) {
        ...
        Person people = new Person("ocean", "man", hobby, site);

        //这里的引用指向特定对象，就可以把引用当成特定对象，通过引用取调用对象的toString方法
        System.out.println(people);

        //通过引用修改site对象内容
        site.setProvince("Weifang");
