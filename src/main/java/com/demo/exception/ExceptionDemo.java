 ...
 *
 *     检查/编译时异常（check exception）就是在程序编译时就会抛出的异常(编译器抛出); 编译时异常类全部直接继承Exception,也就是Exception的直接子类都是编译时异常
 *       包括：IOException，ClassNotFoundException
 *     运行异常（runtime exception）是在程序运行阶段由JVM抛出的异常;运行时异常都是直接继承RunTimeException
 ...
 *  异常处理方式：程序自动抛出异常类对象， throw new Exception()手动抛出异常类对象,  throws方法声明抛出异常类， try-catch捕获处理异常类对象
 *
...
public class ExceptionDemo {
    ...
    /**
     * JVM默认异常处理方式就是打印异常栈信息，并且中断程序运行
     * 程序抛出异常而没有被处理会导致程序提前结束，从而使得后面的代码不能被执行
     */
    @Test
    public void test() {
        //程序执行到这里时候发生异常时间，JVM会自动创建ArithmeticException类型的对象，这个对象包含了详细的异常信息，然后JVM根据异常对象在call stack寻找exception handler
        // JVmM没有找到合适的的exception handler(异常事件没有被处理)， 程序执会在异常对象产生的位置中断，the runtime system (the program) terminates，打印异常信息到控制台上
        //然后由于异常事件没有被处理，
    ...
    public void test2() {
        ...
        System.out.println("Rest of the code can be executed due to exception handling");
