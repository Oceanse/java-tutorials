 ...
 * 若程序抛出的是编译时异常，上层方法在编译期间就必须选择继续往上抛或者处理掉，若在最上层的方法还没有进行处理，那么最终会交给jvm处理：打印异常信息并中断程序
 * 若程序抛出的是运行时异常RuntimeException或它们的子类或者Error，上层方法可以不处理也不抛出,编译仍能顺利通过，但在运行时会被系统抛出
 * 通常，应该catch捕获那些知道如何处理的异常，将不知道如何处理的异常继续传递下去,传递异常可以在方法签名处使用 throws 关键字声明可能会抛出的异常。
 *
 * throws vs throw:
