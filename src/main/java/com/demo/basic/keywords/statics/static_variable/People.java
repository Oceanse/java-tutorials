 ...
 *      注意：类变量只能定义在类内方法外, 即使是类方法内部也不能定义静态变量,因为静态变量在类加载的时候就会被开辟空间初始化，静态方法只有在被调用的时候才会被执行，
 *           也就是如果静态方法内部定义了静态变量，只有静态方法被调用的时候才能初始化其内部定义的静态变量，这就违背了静态变量在类加载的时候就会被创建初始化
 *
 * 存储位置：方法区中,被类的所有实例共享,可以节省内存，使程序存储器高效；另外可以统一修改所有对象的同一静态属性
 * 方法区补充：方法区与 Java 堆一样，是各个线程共享的内存区域，它用于存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码等数据。
 * 虽然 Java 虚拟机规范把方法区描述为堆的一个逻辑部分，但是它却有一个别名叫做 Non-Heap（非堆），目的应该是与 Java 堆区分开来。

 * 初始化：
 * JVM会在加载字节码文件的时候静态变量进行空间分配以及初始化(参考饥饿单例模式)，当系统创建该类的对象时，系统不会再为类变量分配内存，
 * 也不会再次对类变量进行初始化(和静态代码块相似)
 *
 ...
 *
 * 注意：
 * 1 对static关键字而言，有一条非常重要的规则：类成员（包括成员变量、方法、初始化块、内部类和内部枚举）
...
public class People {
    ...
    /**
     * JVM会在加载字节码文件的时候静态变量进行空间分配以及初始化
     * test2方法执行之前会首先通过空参构造创建People对象，然后在开始执行test2方法,所以一共会创建三个对象
     */
    @Test
    public void test2() {
        People people = new People("ocean", 100);
        People people1 = new People("ocean", 120);
