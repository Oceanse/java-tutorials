 ...
 *
 * 数组也是一种数据类型，它本身是一种引用类型。例如int是一个基本类型，但int[]就是一种引用类型了, String[] names中names就是一种引用类型变量，指向数组对象。
 * 数组变量是一个引用变量，存在于栈中，保存的是数组首元素的内存地址或者理解为指向堆内存中对应的数组对象
 *
 ...
 * 数组元素都有下标，范围从0到length-1,数组通过下标访问数组元素,查询效率很高
 *
 */
public class ArrayDemo {
     ...
     */
    @Test
    public void testAllocateMemoryAndAssign() {
     ...
     */
    @Test
    public void testAllocateMemoryAndAssign2() {
        ...
        System.out.println("fruits2.length = " + fruits2.length);
        fruits[0]=new Fruit(new String[]{"apple"});//ArrayIndexOutOfBoundsException: Index 0 out of bounds for length 0
    ...
    /**
     * 如果堆内存中数组不再有任何引用变量指向自己，则这个数组对象将成为垃圾， 该数组所占的堆内存将会被系统的垃圾回收器回收;
     * 看待一个数组时，一定要把数组看成两个部分： 一部分是数组引用，也就是在代码中定义的数组引用变量；
     * 还有一部分是实际的数组对象,这部分是在堆内存里运行的，通常无法直接访问它，只能通过数组引用变量来访问
     */
    @Test
    public void testArrayReference2() {
        Object[] o=new Object[]{ Double.valueOf(1.0), new Resource()} ;
     ...
     */
    @Test
    public void testArrayReference3() {
        double[] scores=new double[]{120.0,130.0,140.0};
        ...
        FRUITS[0]="grape";//首先创建一个新的字符串，然后第一个元素会指向常量池中新的字符串
        System.out.println(Arrays.toString(FRUITS));
    }

    /**
     * 数组元素类型必须是数组引用类型或者数组引用类型的子类型
     */
    @Test
    public void testArrayType() {
        //数组元素类型必须属于Number类或者其子类
        //而number只是引用类型变量，指向堆内存中的首个数组对象的地址
        Number[] number=new Number[]{ Double.valueOf(1.0),  Integer.valueOf(1)} ;
    }
    ...
    public void testArrayType2() {
        ...
        Number[] numbers;
        numbers = integers;
        numbers[0]=1.0;//这里会产生运行是异常：java.lang.ArrayStoreException: java.lang.Double
    }
    ...
    public void testArrayType3() {
        ...
        Class<?> componentType = numbers.getClass().getComponentType();
        System.out.println("componentType = " + componentType.getName());

        Number[] numbers2 = new Double[]{1.0,2.0,3.0};
        Class<?> componentType2 = numbers2.getClass().getComponentType();
        System.out.println("componentType2 = " + componentType2.getName());

        Number[] numbers3=new Number[]{ Double.valueOf(1.0),  Integer.valueOf(1)};
        Class<?> componentType3 = numbers3.getClass().getComponentType();
        System.out.println("componentType3 = " + componentType3.getName());
    }
     ...
     */
    @Test
    public void testArrayReference4() {
        ...
        mouses= new Mouse[]{m1, m2};

        //通过数组元素或者m1,m2访问对象时是相同的
        System.out.println(mouses[0].getColor());
        System.out.println(m1.getColor());

        //通过数组元素或者m1,m2修改Mouse对象时是等价的
        mouses[0].setColor("blue");
        System.out.println(mouses[0].getColor());
        System.out.println(m1.getColor());

        //通过数组元素或者m1,m2修改Mouse对象时是等价的
        m2.setColor("red");
        System.out.println(mouses[1].getColor());
    ...
    public void testArrayAccess2() {
        System.out.println(new double[]{1,2,3}[0]);
        System.out.println(new double[]{1,2,3}[1]);
        System.out.println(new double[]{1,2,3}[2]);
        //编译程序不会出现任何错误，但运行时出现异常： java.lang.ArrayIndexOutOtBoundsException
        System.out.println(new double[]{1,2,3}[3]);
    }
    ...
    public void testArrayAccess3() {
        System.out.println("before-after".split("-")[0]);
        System.out.println("before-after".split("-")[1]);
    }
     ...
     */
    @Test
    public void testArrayTraverse3() {
     ...
     */
    @Test
    public void testArrayTraverse4() {
     ...
     */
    @Test
    public void testArrayTraverse5() {
     ...
     */
    @Test
    public void testArrayTraverse6(){
        ...
        System.out.println(Arrays.toString(new Double[3]));
        System.out.println(Arrays.toString(new Boolean[3]));
        System.out.println(Arrays.toString(new Character[3]));
     ...
     */
    @Test
    public void testArrayTraverse7() {
    ...
    public void testTwoDimensionArray() {
        ...
        double[][] d = new double[][]{{1, 2, 3}, {4, 5, 6}};//=左边声明部分不允许指定长度
