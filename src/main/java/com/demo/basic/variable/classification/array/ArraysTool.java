...
public class ArraysTool {
    ...
    @Test
    public void testMyArraysToString() {
        boolean[] b = new boolean[]{true, false, true};
    ...
    public void testArrayToList() {
        ...
        String[] fruitsArray = new String[]{"apple", "banana", "grape"};
        //传入数组对象的引用
        List<String> fruitsList = Arrays.asList(fruitsArray);
        System.out.println(fruitsList);

        //传入可变参数
        List<String> vegetables = Arrays.asList("tomato", "mushroom", "pepper");
    ...
    public void testSort5() {
        ...
        System.out.println(Arrays.toString(students));
        //通过匿名内部类创建比较器参数
        Arrays.sort(students, (students1,  students2) -> {
            int num = students1.getAge() - students2.getAge();
            return num == 0 ? students1.getName().compareTo(students2.getName()) : num;
                }
        );
        System.out.println(Arrays.toString(students));
    }
    ...
    public void testCopyOf() {
        ...
        String[] newNames = Arrays.copyOf(names, 3);//创建了一个长度是3的新的数组，并把旧数组的元素拷贝到新的数组
        String[] newNames2 = Arrays.copyOf(names, 5); //数组扩容，创建了一个长度是5的新的数组，并把旧数组的元素拷贝到新的数组
        System.out.println(Arrays.toString(newNames));
        System.out.println(newNames==names);
...
//       double[] scores=new double[]{148,150};
//        double[] doubles = Arrays.copyOf(scores, scores.length);
//        Arrays.copyOf(scores, scores.length,double[].class)
//    }


    /**
     * clone前后的两个数组的对地址不同，内容相同
     */
    @Test
    public void testArrayClone() {
        int[] arraySrc = new int[]{3, 1, 8, 2};
        int[] arrayDest = arraySrc.clone();
        System.out.println(arraySrc);
        System.out.println(arrayDest);
        System.out.println(arraySrc==arrayDest);
