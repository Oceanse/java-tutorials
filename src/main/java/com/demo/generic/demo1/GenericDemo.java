 ...
 * 泛型类、泛型接口的属性类型和方法的参数、返回值可以使用生命泛型变量(类型形参)，我们可以称这些属性或者方法为“带泛型的属性方法”或者“使用泛型的属性方法”，因此带泛型的属性方法必须出现在泛型类中;
 * 泛型方法或者泛型属性和泛型类/泛型接口无关，他们可以出现在非泛型类中,也就是泛型方法独立于泛型类或者泛型接口
 * <p>
 *
 *
 * 举例：
...
public class GenericDemo {
    ...
    @Test
    public void testArrayListGeneric() {
    ...
    public void testIteratorGeneric() {
        ...
        while (iterator.hasNext()) {
            //没有使用泛型，会返回Object类型,使用泛型后可以使用泛型类型接收
            Fruit fruit = iterator.next();
            fruit.eat();
        }

        Iterator<Fruit> iterator2 = fruits.iterator();
