...
import com.demo.basic.method.equals_tostring_cloneable.tostring.package1.Apple;
import com.demo.collection_map.model.Product;

/**
 * 子类继承泛型类或者实现泛型接口有两种方式：
 * 若子类是泛型类，那么子类中至少一个类类型形参/类型变量要和父类的类类型形参/类型变量一致: class GenericSub1<T> extends Wildcard<T>
...
/**
 * 子类继承泛型父类时候，若子类是泛型类，那么子类中至少一个类型形参/类型变量要和父类的类型形参/类型变量一致
 * 把父类中使用泛型的属性方法脑补到子类当中
 * @param <T>
 * @param <E>
 */
