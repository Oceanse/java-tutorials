 ...
 * 这样就实现了一个客户端一个线程的模型，
 *
 * 同步和异步(synchronous/asynchronous)：针对主调方，主调方调用方法后，需要等待就是同步，不需要等待，调用后转马上做其他事情就是异步；
 ...
 *
 *阻塞与非阻塞（blocking/non-blocking):针对被调方，被调方法被调用后立即返回就是非阻塞
            ...
            new Thread(() -> {
                try {
                    ...
                    while ((message = bufferedReader.readLine()) != null) {
                        System.out.println("服务端收到 " + socket.getPort()+" 消息："+message);
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
